...
// --- resume command: re-run last failed build ---
program
  .command('resume')
  .description('Resume the last failed build from where it stopped')
  .action(async () => {
    const { readFile: rf } = await import('node:fs/promises');
    const baseDir = process.cwd();
    const anvilDir = join(baseDir, '.anvil');

    // Read the last plan
    let plan;
    try {
      const planJson = await rf(join(anvilDir, 'roadmap.json'), 'utf-8');
      plan = JSON.parse(planJson);
    } catch {
      console.log(chalk.red('No previous build found. Run "anvil run" first.'));
      process.exit(1);
    }

    // Read the last judge reports to find where it failed
    const { readdirSync } = await import('node:fs');
    const reportFiles = readdirSync(join(anvilDir, 'reports')).filter(f => f.startsWith('wave-'));
    if (reportFiles.length === 0) {
      console.log(chalk.red('No wave reports found. Run "anvil run" first.'));
      process.exit(1);
    }

    const lastReport = reportFiles.sort().pop()!;
    const lastReportData = JSON.parse(await rf(join(anvilDir, 'reports', lastReport), 'utf-8'));
    const failedChecks = lastReportData.checks?.filter((c: any) => !c.passed) ?? [];

    console.log(chalk.bold('\nAnvil Resume\n'));
    console.log(`  Last build: ${plan.spec?.slice(0, 60) ?? 'unknown'}...`);
    console.log(`  Failed at:  Wave ${lastReportData.waveNumber}`);
    if (failedChecks.length > 0) {
      console.log(`  Failures:`);
      for (const c of failedChecks) {
        console.log(chalk.red(`    - ${c.name}: ${c.message ?? 'FAILED'}`));
        if (c.details) console.log(chalk.dim(`      ${c.details.slice(0, 150)}`));
      }
    }
    console.log();
    console.log(chalk.yellow('To re-run with the same spec:'));
    console.log(chalk.dim(`  anvil run "${plan.spec?.slice(0, 80)}" --skip-review`));
    console.log();
    console.log(chalk.yellow('To fix manually, check:'));
    console.log(chalk.dim(`  anvil logs --wave ${lastReportData.waveNumber}`));
  });

// --- pr command: create GitHub PR with audit summary ---
program
  .command('pr')
  .description('Create a GitHub PR with build audit summary as description')
  ...
  .action(async (opts: Record<string, string>) => {
    ...
    let plan, costReport, highCourtReport;
    try {
      plan = JSON.parse(await rf(join(anvilDir, 'roadmap.json'), 'utf-8'));
      costReport = JSON.parse(await rf(join(anvilDir, 'cost-report.json'), 'utf-8'));
    } catch {
      console.log(chalk.red('No build artifacts found. Run "anvil run" first.'));
      process.exit(1);
    }
    try {
      highCourtReport = JSON.parse(await rf(join(anvilDir, 'high-court-report.json'), 'utf-8'));
    } catch {
      highCourtReport = null;
    }

    const title = opts.title ?? `feat: ${plan.spec?.slice(0, 60) ?? 'Anvil build'}`;
    const taskList = plan.tasks?.map((t: any) => `- ${t.id}: ${t.description?.slice(0, 80)}`).join('\n') ?? '';
    const verdict = highCourtReport?.verdict ?? 'unknown';
    const cost = costReport?.totalCostUsd?.toFixed(4) ?? 'unknown';

    const body = `## Summary

Built with [Anvil](https://github.com/fepvenancio/anvil) — AI Code Factory.
...
## Quality
- **High Court Verdict:** ${verdict}
- **Total Cost:** $${cost}
- **Sub-Judges:** tsc, vitest, touch-map, security, interface — all passed

## Audit Trail
Full build artifacts in \`.anvil/\` directory.

---
*Generated by Anvil v0.4.1*`;

    try {
      const cmd = `gh pr create --title "${title.replace(/"/g, '\\"')}" --body "$(cat <<'ANVIL_EOF'\n${body}\nANVIL_EOF\n)" --base ${opts.base ?? 'main'}`;
      console.log(chalk.blue('Creating PR...'));
      const result = execSync(cmd, { cwd: baseDir, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
      console.log(chalk.green(`PR created: ${result.trim()}`));
    } catch (err: any) {
      console.log(chalk.red('Failed to create PR. Make sure gh CLI is installed and authenticated.'));
      if (err.stderr) console.log(chalk.dim(err.stderr.slice(0, 200)));
    }
  });
